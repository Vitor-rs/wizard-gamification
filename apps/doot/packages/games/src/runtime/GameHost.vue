...
import { useHostJoinUrl } from './joinUrl'
...
const i18n = inject<{ t: (k: string, p?: any) => string } | null>('dootI18n', null)
const t = (k: string, fallback: string, params?: any) => (i18n ? i18n.t(k, params) : fallback)
...
const joinUrl = useHostJoinUrl(() => room.code.value)
...
const stateLabel = computed(() => {
  const labels: Record<string, string> = {
    ready: t('host.getReady', 'Get ready'),
    open: isJudge.value ? t('host.votingOpen', 'Voting open') : t('host.answersOpen', 'Answers open'),
    locked: isJudge.value ? t('host.votingClosed', 'Voting closed') : t('host.answersIn', 'Answers in'),
    reveal: t('host.results', 'Results'),
  }
...
<template>
    ...
    <section class="panel roster-card">
      <div class="lobby-head">
        ...
        <details v-if="scoringLines.length" class="lobby-scoring">
          <summary>{{ t('host.howScoringWorks', 'How scoring works') }}</summary>
          <ul>
      ...
      <div class="roster-head">
        <div class="kicker">{{ t('host.inTheRoom', 'In the room') }}</div>
        <span class="count mono">
          {{ room.players.value.length }} {{ t('host.joined', 'joined') }}<template v-if="room.audienceCount.value > 0"> · {{ room.audienceCount.value }} {{ t('host.watching', 'watching') }}</template>
        </span>
      </div>
      ...
      <details class="lobby-advanced">
        <summary class="lobby-advanced-sum">{{ t('host.adjustTonight', 'Adjust for tonight') }}</summary>
      <div class="cap-pick">
        <label class="cap-row">
          <input
            type="checkbox"
          ...
          />
          <span class="kicker">{{ t('host.limitJoin', 'Limit how many can join') }}</span>
        </label>
        <input
          v-if="playerCap != null"
          type="number"
      ...
      <div class="cap-pick">
        <label class="cap-row">
          ...
          />
          <span class="kicker">{{ t('host.playInTeams', 'Play in teams') }}</span>
        </label>
        <template v-if="teamsOn">
          ...
          <button type="button" class="btn btn-ghost btn-block" @click="autoBalance">
            {{ t('host.autoBalance', 'Auto-balance players') }}
          </button>
          <p class="note">{{ t('host.teamNote', 'Players pick a team on their phones, or tap Auto-balance to split them evenly.') }}</p>
        </template>
      </div>
      ...
      <label class="cap-row timers-row">
        ...
        />
        <span class="kicker">{{ t('host.turnOffTimers', 'Turn off round timers') }}</span>
      </label>
      <label class="cap-row timers-row">
        <input
          type="checkbox"
        ...
        />
        <span class="kicker">{{ t('host.autoAdvance', 'Advance as soon as everyone has answered') }}</span>
      </label>
      <label class="cap-row timers-row">
        <input
          type="checkbox"
        ...
        />
        <span class="kicker">{{ t('host.sfx', 'Sound effects on this screen') }}</span>
      </label>
      ...
      <div class="cohost-pick">
        <span class="kicker">{{ t('host.whoDrives', 'Who drives the game') }}</span>
        <select
          class="cohost-select"
        ...
        >
          <option value="">{{ t('host.justMe', 'Just me (host)') }}</option>
        ...
        <label class="cohost-first">
          ...
          />
          <span>{{ t('host.firstDrives', 'Let the first to join drive') }}</span>
        </label>
      </div>
      <div class="lobby-actions">
        <DButton variant="primary" size="lg" :disabled="!config" @click="startGame">
          {{ t('host.startGame', 'Start game') }}
        </DButton>
      </div>
      <p class="note">
        {{ t('host.lateJoinNote', 'Players who join after you start can only play rounds from when they joined.') }}
      </p>
    </section>
  </div>
  ...
  <div v-else-if="room.phase.value === 'results' && room.results.value" class="results-wrap">
    ...
    <div v-if="!sessionMode" class="results-next">
      <button type="button" class="btn btn-primary btn-lg" @click="playAgain">{{ t('host.playAgain', 'Play again') }}</button>
      <button v-if="hostNewRoom" type="button" class="btn btn-ghost btn-lg" @click="hostNewRoom">{{ t('host.newRoom', 'New room') }}</button>
      <a class="btn btn-ghost btn-lg" href="/explore">{{ t('host.pickAnother', 'Pick another game') }}</a>
      <a class="btn btn-ghost btn-lg" href="/">{{ t('host.home', 'Home') }}</a>
      <p v-if="hostNewRoom" class="results-hint">
        <b>{{ t('host.playAgain', 'Play again') }}</b> {{ t('host.playAgainHint', 'keeps this crowd and resets scores. New room starts a fresh group.') }}
      </p>
    </div>
  </div>
  ...
  <div v-else-if="instance && block" class="stage">
    ...
    <div v-if="driverName" class="driving-note">
      <span class="dn-label"><Icon name="mc" :size="16" /> {{ t('host.isDriving', '{name} is driving from their phone', { name: driverName }) }}</span>
      <button type="button" class="take-back" @click="room.host.setDriver(null)">{{ t('host.takeBack', 'Take back') }}</button>
    </div>
    </div>
    ...
    <ControlBar
      ...
      :round-label="t('host.round', 'Round')"
      :locked-in-label="t('host.lockedIn', 'locked in')"
      :in-label="t('host.allIn', 'in ✓')"
    ...
    >
      ...
      <DButton v-if="isDisplay" variant="primary" size="lg" @click="advanceDisplay()">
        {{ isLast ? t('host.finalResults', 'Final results') : t('host.next', 'Next →') }}
      </DButton>
      <DButton v-else-if="room.host.can('open')" variant="primary" size="lg" @click="room.host.openVoting()">
        {{ isMakeRound ? t('host.collectAnswers', 'Collect answers') : t('host.openVoting', 'Open voting') }}
      </DButton>
      <DButton v-else-if="room.host.can('lock')" @click="room.host.lock()">
        {{ isMakeRound ? t('host.lockAnswers', 'Lock answers') : t('host.lockVoting', 'Lock voting') }}
      </DButton>
