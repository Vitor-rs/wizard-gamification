...
<template>
  <div class="player">
    ...
    <div v-else-if="room.phase.value === 'lobby'" class="big">
      <h2>{{ t('player.youAreIn', 'You are in!') }}</h2>
      <div v-if="teams.length" class="team-pick">
        <p class="team-pick-label">{{ room.myTeam.value ? t('player.yourTeam', 'Your team') : t('player.pickTeam', 'Pick your team') }}</p>
      ...
      <template v-if="room.isDriver.value">
        <p><Icon name="mc" :size="15" /> {{ t('player.mcLead', "You're the MC. Kick it off when everyone's in.") }}</p>
        <button class="btn btn-primary btn-block" @click="room.sendControl('start')">{{ t('player.startGame', 'Start game →') }}</button>
      </template>
      <p v-else>{{ t('player.waitingHost', 'Waiting for the host to start. Keep this page open.') }}</p>
    </div>
    ...
    <template v-else-if="room.phase.value === 'results' && room.results.value">
      ...
      <component
        ...
        compact
      />
      <a class="btn btn-ghost btn-block" href="/">{{ t('join.backToStart', 'Back to start') }}</a>
      <ReportButton
    ...
    <div v-else-if="state === 'open' && submitted" class="big">
      ...
      <h2>{{ t('player.lockedIn', 'Locked in') }}</h2>
      <p>{{ t('player.waitingOthers', 'Waiting for everyone else…') }}</p>
    </div>
    ...
    <div v-else class="big">
      <h2>{{ t('player.resultsUp', 'Results are up!') }}</h2>
      <p>{{ t('player.watchScreen', 'Check the big screen.') }}</p>
    </div>
