...
<template>
  <PhoneShell>
    ...
    <div v-else-if="notFound" class="loading deadroom">
      <h2>{{ t('join.notFound', { code }) }}</h2>
      <p>{{ t('join.notFoundDesc') }}</p>
      <NuxtLink to="/" class="btn btn-primary">{{ t('join.backToStart') }}</NuxtLink>
    </div>
    <div v-else class="loading">{{ t('join.joiningRoom', { code }) }}</div>
  </PhoneShell>
</template>
