    ...
    <template v-if="loggedIn">
      ...
      <Transition name="dd">
        <div v-if="open" ref="menuRef" class="menu" role="menu" @keydown="onMenuKeydown">
          ...
          <button type="button" class="menu-item logout" role="menuitem" @click="logout">{{ t('nav.logout') }}</button>
        </div>
      </Transition>
    </template>

    <template v-else>
      <NuxtLink to="/login" class="navlink">{{ t('nav.login') }}</NuxtLink>
      <NuxtLink to="/login?signup=1" class="signup-btn">{{ t('nav.signup') }}</NuxtLink>
    </template>
  </div>
</template>
