        ...
        <div class="panel card">
          <div class="gate-header">
            ...
            <button
              type="button"
              class="lang-pill"
              :title="locale === 'pt-BR' ? 'Mudar para Inglês' : 'Switch to Portuguese'"
              @click="toggleLocale"
            >
              {{ locale === 'pt-BR' ? '🇧🇷 PT' : '🇺🇸 EN' }}
            </button>
          </div>
          <p class="lead">{{ t('join.joiningRoom', { code: roomCode }) }}</p>
          <JoinForm
            :initial-code="roomCode"
            :initial-name="savedName"
            :probe="probeName"
            :labels="{
              roomCode: t('join.roomCode'),
              roomCodePlaceholder: t('join.codePlaceholder'),
              yourName: t('join.yourName'),
              namePlaceholder: t('join.namePlaceholder'),
              joinGame: t('join.joinGame'),
              justWatch: t('join.justWatch'),
              checking: t('join.checking'),
              roomFullMsg: t('join.roomFullDesc'),
              watchInstead: t('join.watchAudience'),
              tryAgain: t('join.checking'),
              nameTakenMsg: t('join.nameTakenDesc'),
              reconnect: t('join.reconnect'),
              pickDifferent: t('join.pickDifferent'),
              errCode: t('join.enterCodeErr'),
              errName: t('join.enterNameErr'),
            }"
            @join="onJoin"
            @watch="onWatch"
          />
