...
import { createOrRotatePersonalToken, getUserPersonalToken } from '../utils/token'
...
export default defineEventHandler(async (event) => {
  // CORS headers for web-based LLM clients (LibreChat, Open WebUI, custom frontends)
  setResponseHeader(event, 'Access-Control-Allow-Origin', '*')
  setResponseHeader(event, 'Access-Control-Allow-Methods', 'GET, POST, OPTIONS')
  setResponseHeader(event, 'Access-Control-Allow-Headers', 'Content-Type, Authorization, X-API-Key, Accept')
  setResponseHeader(event, 'Access-Control-Expose-Headers', 'WWW-Authenticate')
  ...
  if (event.method === 'GET') {
    ...
    // If opened directly in a web browser, redirect to the universal connect guide
    if (accept.includes('text/html')) {
      return sendRedirect(event, '/connect', 302)
    }
    // Otherwise return MCP discovery info for agentic crawlers / CLI tools
    return {
      name: 'doot',
      title: 'Doot MCP Server',
      description:
        'Model Context Protocol (MCP) server for Doot. Connect any LLM (Claude, ChatGPT, Cursor, Windsurf, Gemini, Ollama, etc.) to build, validate, and save party games directly to your account.',
      version: '0.1.0',
      protocolVersions: ['2025-06-18', '2024-11-05'],
      endpoint: '/mcp',
      transport: 'streamable-http',
      auth: {
        type: 'Bearer / OAuth 2.1',
        oauthDiscovery: '/.well-known/oauth-protected-resource',
        tokenManagement: '/connect',
      },
      tools: TOOLS.map((t) => ({ name: t.name, description: t.description })),
      documentation: `${BASE}/connect`,
    }
  }
  ...
  // Support alternative token passing (X-API-Key or query param ?token=... or session cookie)
  if (!req.headers.has('authorization')) {
    const customKey = getRequestHeader(event, 'x-api-key') || (getQuery(event).token as string | undefined)
    if (customKey) {
      const headers = new Headers(req.headers)
      headers.set('authorization', `Bearer ${customKey}`)
      ...
      // Fallback: check if the user has an active session cookie (e.g. testing in browser or web client)
      try {
        const session = await useAuth().api.getSession({ headers: event.headers })
        if (session?.user?.id) {
          let pat = await getUserPersonalToken(session.user.id)
          if (!pat) pat = await createOrRotatePersonalToken(session.user.id)
          const headers = new Headers(req.headers)
          headers.set('authorization', `Bearer ${pat.token}`)
          effectiveReq = new Request(req.url, {
            method: req.method,
            headers,
            body: req.body,
  ...
  return mcpHandler(effectiveReq)
})
