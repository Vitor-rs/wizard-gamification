...
export default defineNitroPlugin(async () => {
  ...
  // Ensure system application for Personal Access Tokens / Multi-LLM keys exists.
  try {
    await client.execute({
      sql: `INSERT OR IGNORE INTO oauthApplication (id, name, clientId, clientSecret, redirectUrls, type, disabled, createdAt, updatedAt)
            VALUES ('app_personal_tokens', 'Personal Access Tokens & Multi-LLM Keys', 'doot-personal-client', '', 'https://localhost', 'public', 0, ?, ?)`,
      args: [new Date().toISOString(), new Date().toISOString()],
    })
  } catch {
    // Expected if table not yet created
  }
})
