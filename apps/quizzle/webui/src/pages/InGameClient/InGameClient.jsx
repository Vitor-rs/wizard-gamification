...
export const InGameClient = () => {
    ...
    useEffect(() => {
        if (practiceCode) {
            ...
            if (!practiceUserData || !practiceUserData.name) {
                toast.error('Por favor, escolha um nome e um personagem primeiro.');
        ...
        const hostDisconnected = () => {
            clearCurrentSession();
            toast.error("O anfitrião saiu do jogo.", {
                duration: 3000
            });
        ...
        const handleReconnection = (success, error, gameState) => {
            if (success) {
                ...
                if (error === 'Session expired' || 
                    ...
                    let message = "Sessão expirada. Retornando ao início...";
                    if (error === 'Kicked from room') {
                        message = "Você foi removido da sala. Retornando ao início...";
                    } else if (error === 'Host disconnected') {
                        message = "O anfitrião saiu do jogo. Retornando ao início...";
                    }
                    
                    toast.error(message, { duration: 2000 });
                    setTimeout(() => navigate("/"), 500);
                } else {
                    toast.error("Conexão interrompida. Tentando reconectar...", { duration: 2000 });
                }
            }
        };
    ...
    const loadPracticeQuiz = async () => {
        try {
            ...
            if (error.message && error.message.includes('410')) {
                toast.error('Este quiz de treino expirou.');
            } else if (error.message && error.message.includes('404')) {
                toast.error('Quiz de treino não encontrado.');
            } else {
                toast.error('Erro ao carregar o quiz de treino.');
            }
            navigate('/');
        }
    };
    ...
    const submitAnswer = (answers) => {
        ...
        if (!answersReady) {
            toast.error("As respostas ainda não estão prontas!");
            return;
        }
        ...
        <div className="ingame-client">
            {!isPracticeMode && (!isConnected || isReconnecting) && (
                <div className="connection-status">
                    ...
                    />
                    <span>{isReconnecting ? 'Reconectando...' : 'Conexão perdida'}</span>
                </div>
            )}
                    ...
                    <div className="ingame-header">
                        {isPracticeMode && (
                            <div className="practice-progress">
                                ...
                                <span>Pergunta {currentQuestionIndex + 1} de {practiceQuiz.questions.length}</span>
                            </div>
                        )}
                    ...
                    {isPracticeMode ? (
                        practiceQuestionResult?.isLastQuestion ? (
                            ...
                            <>
                                ...
                                />
                                <h2>
                                    {practiceQuestionResult?.result === 'correct' ? "Correto!" :
                                     practiceQuestionResult?.result === 'partial' ? "Parcialmente correto!" : "Não foi dessa vez!"}
                                </h2>
                                <ClientAnswerReview
                        ...
                        (() => {
                            ...
                            const statusTitle = status === 1 ? 'Correto!' : status === 0 ? 'Quase lá!' : 'Não foi dessa vez!';
                                ...
                                <div className="result-reveal">
                                        ...
                                        <div className="result-card streak-card">
                                            <div className="streak-card-header">
                                                <div className="streak-card-title">
                                                    ...
                                                    <span className="streak-label">Acertos em sequência</span>
                                                </div>
                                    ...
                                    {rank && (
                                        <div className="result-card rank-card">
                                            <div className="rank-badge">{rank}º</div>
                                            <div className="rank-info">
                                                <span className="rank-main">{rank}º Lugar de {totalPlayers}</span>
                                                <span className="rank-sub">
                                                    {playerAhead && playerAhead.gap >= 0
                                                        ? `${playerAhead.gap} pontos para a próxima posição`
                                                        : rank === 1
