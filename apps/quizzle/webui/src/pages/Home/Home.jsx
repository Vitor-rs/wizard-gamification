            ...
            jsonRequest(`/practice/${code.toUpperCase()}/exists`)
                .then(data => {
                    ...
                    if (data.exists) {
                        ...
                        showError(data.message || "Quiz de treino não encontrado");
                    }
                })
                .catch(error => {
                    setLoading(false);
                    console.error('Error checking practice quiz:', error);
                    showError("Erro ao verificar o quiz de treino");
                });
            return;
        }
            ...
            ensureSocketConnection().then(() => {
                socket.emit("CHECK_ROOM", {code: parseInt(code)}, (response) => {
                    if (response?.success && response?.exists && !response?.isPractice) {
                        ...
                        showError(response?.error || "Sala não encontrada");
                    }
                });
            }).catch(() => {
    ...
    return (
        <div className="home-page">
            <motion.div className="legal-area" initial={{opacity: 0, y: 50}} animate={{opacity: 1, y: 0}}>
                <a href={imprint} target="_blank" rel="noreferrer">Sobre</a>
                <a href={privacy} target="_blank" rel="noreferrer">Privacidade</a>
                <a href="/credits" onClick={(e) => { e.preventDefault(); navigate('/credits'); }}>Créditos</a>
            ...
            <motion.div initial={{opacity: 0, y: 50}} animate={{opacity: 1, y: 0}} transition={{delay: 0.5}}
                ...
                <div className={`action-area ${code !== null ? 'disabled' : ''}`}>
                    <Button text="Criar Quiz" icon={faSwatchbook} padding={"0.8rem 2.5rem"}
                            disabled={code !== null}
                            onClick={() => {
                                setCirclePosition("-30rem 0 0 -30rem");
                                setTimeout(() => navigate("/create"), 500);
                            }}/>
                    <Button text="Hospedar Sala" icon={faShareFromSquare} padding={"0.8rem 2.5rem"}
                            disabled={code !== null}
                            onClick={() => {
                                setCirclePosition("-30rem 0 0 -30rem");
                    ...
                    {isAdmin && (
                        <Button text="Painel Admin" icon={faGear} padding={"0.8rem 2.5rem"} type="secondary"
                                disabled={code !== null}
                                onClick={() => {
                                    setCirclePosition("-30rem 0 0 -30rem");
                    ...
                    {!isAuthenticated && (
                        <Button text="Entrar" icon={faRightToBracket} padding={"0.8rem 2.5rem"} type="secondary"
                                disabled={code !== null}
                                onClick={() => requireAuth(() => {})}/>
                    )}
                    {isAuthenticated && (
                        <Button text="Sair" icon={faRightFromBracket} padding={"0.8rem 2.5rem"} type="secondary"
                                disabled={code !== null}
                                onClick={() => {
                                    logout();
