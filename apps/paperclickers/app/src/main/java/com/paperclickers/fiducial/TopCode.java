 ...
 * fiducials designed to be recognized quickly by low-resolution digital cameras
 * with poor optics. The TopCode symbol format is based on the open SpotCode
 * format:
 * 
 * http://www.highenergymagic.com/spotcode/symbols.html
 * 
 * Each TopCode encodes a 13-bit number in a single data ring on the outer edge
 * of the symbol. Zero is represented by a black sector and one is represented
 * by a white sector.
 * 
 * @author Michael Horn
...
public class TopCode {
	...
	/**
	 * Returns the orientation of this code in radians and accurate to about
	 * plus or minus one degree. This value gets set automatically by the
	 * decode() function.
	 */
	...
	/**
	 * Returns the diameter of this code in pixels. This value will be set
	...
	/**
	 * Returns the x-coordinate for the center point of the symbol. This gets
	 * set automatically by the decode() function.
	 */
	...
	/**
	 * Returns the y-coordinate for the center point of the symbol. This gets
	 * set automatically by the decode() function.
	 */
	...
	/**
	 * Decodes a symbol given any point (cx, cy) inside the center circle
	 * (bulls-eye) of the code.
	 */
	public int decode(Scanner scanner, int cx, int cy) {

		// Measure distance from candidate center (cx, cy) until first color transition in four directions
		// (averages three measures per direction)
		int up = (scanner.ydist(cx, cy, -1) + scanner.ydist(cx - 1, cy, -1) + scanner
				.ydist(cx + 1, cy, -1));

		int down = (scanner.ydist(cx, cy, 1) + scanner.ydist(cx - 1, cy, 1) + scanner
				.ydist(cx + 1, cy, 1));

		int left = (scanner.xdist(cx, cy, -1) + scanner.xdist(cx, cy - 1, -1) + scanner
				.xdist(cx, cy + 1, -1));

		int right = (scanner.xdist(cx, cy, 1) + scanner.xdist(cx, cy - 1, 1) + scanner
				.xdist(cx, cy + 1, 1));

		// Recentralize bullseye center
		this.x = cx;
		this.y = cy;
		this.x += (right - left) / 6.0f;
		this.y += (down - up) / 6.0f;
		
		// Calculates the UNIT size for this given topcode candidate
		...
		// In this outer loop tries UNIT adjustments of -10% until 10% of
		// calculated UNIT size
		
		for (int u = -2; u <= 2; u++) {
			
			// In this inner loop tries arc rotation adjustments of 0% until 90% of
			// standard arc size
	 ...
	 * 
	 * scanner - image scanner unit - width of a single ring (codes are 8 units
	 * wide) arca - Arc adjustment. Rotation correction delta value.
	 */
	protected int readCode(Scanner scanner, float unit, float arca) {
		...
		// Loop through all the data ring sectors (13)
		...
		for (int sector = SECTORS - 1; sector >= 0; sector--) {
			...
			for (int i = 0; i < WIDTH; i++) {
				...
				sx = Math.round(x + dx * dist);
				sy = Math.round(y + dy * dist);
				
				core[i] = scanner.getSample3x3(sx, sy);
			}

			// white rings
			if (core[1] <= 128 || core[3] <= 128 || core[4] <= 128  
					|| core[6] <= 128) {
				return 0;
			}
			...
			// data rings: greater confidence for extreme values (0 = black, 255 = white)
			c += Math.abs(core[7] * 2 - 0xff);

			// opposite data ring: greater confidence for uncertain (middle) values (128)
	...
	/**
	 * rotateLowest() tries each of the possible rotations and returns the
	 * lowest.
	 */
	...
	/**
	 * Determines the symbol's unit length by counting the number of pixels
	 * between the outer edges of the first black ring. North, south, east, and
	 * west readings are taken and the average is returned.
	 */
	protected float readUnit(Scanner scanner) {
		...
		for (int i = 1; true; i++) {
			...
			// Check if it has not gone outside image limits
			...
			// Check to only save the distance once...				
			...
			if (distL <= 0) {				
				if (whiteL && sample == 0) {
					...
					// The distance is saved when the first white pixel
					// is found after a black region
			...
			if (distR > 0 && distL > 0 && distU > 0 && distD > 0) {
				...
				// Averages by 8 because has gone two units in each one of the
				// 4 directions
				
				float u = (distR + distL + distU + distD) / 8.0f;
				
				// Make sure it is a reasonable circle
	...
	/**
	 * Debug routine that prints the 13 least significant bits of a integer.
	 */
