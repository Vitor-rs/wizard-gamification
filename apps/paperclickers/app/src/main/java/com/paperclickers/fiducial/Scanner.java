...
/**
 * Loads and scans images for TopCodes. The algorithm does a single sweep of an
 * image (scanning one horizontal line at a time) looking for a TopCode bullseye
 * patterns. If the pattern matches and the black and white regions meet certain
 * ratio constraints, then the pixel is tested as the center of a candidate
 * TopCode.
 * 
 * @author Michael Horn
 */
public class Scanner {
	...
	static final String LOG_TAG = "Scanner"; 
	...
	/**
	 * Sets the maximum allowable diameter (in pixels) for a TopCode identified
	 * by the scanner. Setting this to a reasonable value for your application
	 * will reduce false positives (recognizing codes that aren't actually
	 * there) and improve performance (because fewer candidate codes will be
	 * tested). Setting this value to as low as 50 or 60 pixels could be
	 * advisable for some applications. However, setting the maximum diameter
	 * too low will prevent valid codes from being recognized. The default value
	 * is 640 pixels.
	 */
	...
	/**
	 * Average of thresholded pixels in a 3x3 region around (x,y). Returned
	 * value is between 0 (black) and 255 (white).
	 */
	...
	/**
	 * Perform Wellner adaptive thresholding to produce binary pixel data. Also
	 * mark candidate spotcode locations.
	 * 
			...
			for (int i = 0; i < w; i++) {
				if ((data[k] & 0x2000000) > 0) {
					if ((data[k - 1] & 0x2000000) > 0
							...
							&& (data[k + w] & 0x2000000) > 0) {
						/*
						 * if ((data[k-w] & 0x2000000) > 0 || (data[k+w] &
						 * 0x2000000) > 0)) {
						 */
	 ...
	 */
	protected TopCode overlaps(List<TopCode> spots, int x, int y) {
	...
	/**
	 * Counts the number of vertical pixels in direction (d) from (x,y) until a color change is
	 * perceived.
	 */
